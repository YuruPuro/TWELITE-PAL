...
//      dp += 4 ;
  ...
  Serial.print(twelitePalEnv[envNo].temperature) ;
  Serial.print("  humidity=") ;
  Serial.print(twelitePalEnv[envNo].humidity) ;
  Serial.print("  brightness=") ;
  Serial.println(twelitePalEnv[envNo].brightness) ;
#endif
...
//  M5.Lcd.fillRect(0, yP , 320, TEXT_HEIGHT*5 , TFT_BLACK);  // なくても平気なので、チラツキ防止のために外す
  for (int dn=0;dn < PALNUM ; dn ++) {
    M5.Lcd.setTextColor(TFT_GREENYELLOW, TFT_BLACK);

    // -----
    // 送信デバイスID
    xP = M5.Lcd.drawString("Dev:", 0 , yP , 4);
    hexCharaSet(twelitePalEnv[dn].devId,dispStr) ;
    xP += M5.Lcd.drawString(dispStr, xP, yP, 4);

    // 電波強度
    xP += M5.Lcd.drawString("/LQI:", xP , yP , 4);
    sprintf(dispStr,"%03d",twelitePalEnv[dn].lqi) ;
    xP += M5.Lcd.drawString(dispStr, xP, yP, 4);

    // 電圧
    xP += M5.Lcd.drawString("/Vcc:", xP , yP , 4);
    float vcc = twelitePalEnv[dn].volt / 1000. ;
    ...
    sprintf(dispStr,"%d Lux  ",twelitePalEnv[dn].brightness) ;
    xP += M5.Lcd.drawString(dispStr, xP, yP, 4);

    // --- 温湿度算出
    float humidityDisp = twelitePalEnv[dn].humidity / 100. ;
    float temperatureDisp = twelitePalEnv[dn].temperature /100.;
